    ...
    try:
        async with DefaultAzureCredential() as credential:
            ...
            async with sb_client:
                ...
                receiver = sb_client.get_queue_receiver(queue_name=archive_retro_queue_name,
                                                        receive_mode=ServiceBusReceiveMode.RECEIVE_AND_DELETE)
                logger.debug('After Receiver is created....')
                async with receiver:
                    logger.debug(f'Receiver Active on {archive_retro_queue_name}')
                    async for msg in receiver:
                        try:
                            ...
                            logger.debug(f'Message Processed from {archive_retro_queue_name}....')
