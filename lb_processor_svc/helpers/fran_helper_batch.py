...
def proc_store_rec_batch(store_id: str, rest_no: str, fran_json: Dict,
                         store_cont_json: Dict) -> Dict:
    franchisee_df = pd.DataFrame(fran_json)
    store_rec_json = evaluate_store_rec(store_id, rest_no, pd.DataFrame(store_cont_json))
...
def create_fran_container_batch(store_id: str, rest_no: str, store_container_json: Dict):
    new_store_json = evaluate_store_rec(store_id, rest_no, pd.DataFrame(store_container_json))
    df = pd.DataFrame([new_store_json])
    return json.loads(df.to_json(orient='records'))


def evaluate_store_rec(store_id: str, rest_no: str, store_cont_df: DataFrame) -> Dict:
